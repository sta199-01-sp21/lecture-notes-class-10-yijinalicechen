...
the sum of all the rows and columns-- 84 patients.
...
my guess is that the probability of the square of dice roll 1 equaling to dice roll 2 is higher.
...
```{r identify_events1}
roll_results_square <- tibble(
  roll_1= sample (dice, size=10000, replace=TRUE),
  roll_2= sample (dice, size=10000, replace=TRUE))
roll_results_square

roll_results_square <- roll_results_square %>% 
  mutate(square_match = roll_1 == roll_2)
roll_results_square
```

```{r identify_events2, warning=FALSE}
roll_results_sum <- tibble(
  roll_1= sample (dice, size=10000, replace=TRUE),
  roll_2= sample (dice, size=10000, replace=TRUE))
roll_results_sum

roll_results_sum %>%
  mutate(total_sum = strtoi(roll_1, base = 0L) + strtoi(roll_2, base = 0L))
roll_results_sum$total_sum
```
...
```{r summarize_rolls2, include=FALSE}
roll_results_sum 
# i have some problems since R keeps telling me the total_sum column is unknown
```

I think the proportion of total sum equaling 4 is actually bigger.
